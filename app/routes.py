...
# ----------------------------------------------------
# 2. DASHBOARD BGH, XUẤT EXCEL & CHỨC NĂNG DUYỆT TỔ TRƯỞNG (HIỆU PHÓ)
# ----------------------------------------------------
@app.route("/admin/dashboard")
def admin_dashboard():
    try:
        bgh_keywords = ["Hữu", "Hùng", "Kiệt"]
        ...
        total_teachers = len(official_teachers) if official_teachers else 34
        ...
        total_teachers, total_approved, total_pending, school_progress = 34, 0, 0, 0
        dept_stats = [
            {"name": "Tổ Tự Nhiên", "teacher_count": 14, "leader": "TRẦN NHƯ THUỶ", "approved": 0, "pending": 0, "progress": 0},
            {"name": "Tổ Xã Hội", "teacher_count": 7, "leader": "Chưa phân công", "approved": 0, "pending": 0, "progress": 0},
            {"name": "Tổ Tổng Hợp", "teacher_count": 5, "leader": "Chưa phân công", "approved": 0, "pending": 0, "progress": 0},
            {"name": "Tổ Ngoại Ngữ", "teacher_count": 4, "leader": "Chưa phân công", "approved": 0, "pending": 0, "progress": 0},
        ]
...
# DÀNH RIÊNG CHO HIỆU PHÓ (HP): TRANG THẨM ĐỊNH MINH CHỨNG TỔ TRƯỞNG
@app.route("/admin/review-heads")
def admin_review_heads():
    teacher_id = session.get("teacher_id")
    role_code = session.get("role_code")

    if not teacher_id or role_code != "HP":
        flash("Trang này dành riêng cho Hiệu phó duyệt minh chứng Tổ trưởng!", "warning")
        return redirect(url_for("admin_dashboard"))

    tt_roles = Role.query.filter_by(code="TT").all()
...
def my_criteria():
    ...
    for r in records:
        ...
        criteria_list.append({
            ...
            "submitted_at": getattr(r, "submitted_at", None),  # 👈 Bổ sung thời gian nộp
            "reviewed_at": getattr(r, "reviewed_at", None),    # 👈 Bổ sung thời gian duyệt
            "evidences": ev_list
        })
...
# 2. Thêm Minh Chứng Mới & Ghi Nhận Thời Gian Nộp
@app.route("/teacher/submit-evidence", methods=["POST"])
def submit_evidence():
    ...
    if tc:
        ...
        if file_path or url:
            ...
            # 👇 ĐOẠN ĐƯỢC CHÈN VÀO ĐÂY:
            tc.status = "DA_NOP"
            tc.submitted_at = datetime.now()  # Ghi nhận thời gian nộp minh chứng
            tc.feedback = None
            db.session.commit()
    ...
    if tc and tc.teacher_id == teacher_id:
        ...
        if tc_ev:
            ...
            if ev and ev.file_path:
                abs_path = os.path.join(app.root_path, ev.file_path.replace("/", os.sep))
...
# ----------------------------------------------------
# 4. TỔ TRƯỜNG THẨM ĐỊNH & XỬ LÝ TRẠNG THÁI
# ----------------------------------------------------
# 4. Thẩm định minh chứng & Ghi nhận thời gian duyệt
...
def update_status():
    ...
    if tc:
        ...
        tc.reviewed_at = datetime.now()  # Ghi nhận thời gian duyệt/yêu cầu sửa
        db.session.commit()

    # Tự động quay lại trang vừa thao tác (Duyệt Tổ Trưởng của HP hoặc Duyệt Tổ của TT)
    return redirect(request.referrer or url_for("department_review"))

# Trang Thẩm Định Minh Chứng Cho Tổ Trưởng
# Trang Thẩm Định Minh Chứng Cho Tổ Trưởng
# Trang Thẩm Định Minh Chứng Gom Nhóm Theo Giáo Viên
...
def department_review():
    ...
    if not current_teacher or not current_teacher.role or current_teacher.role.code != "TT":
    ...
    # Lấy danh sách giáo viên trong cùng tổ (trừ Tổ trưởng)
    ...
    for teacher in teachers_in_dept:
        # Lấy tiêu chí đã nộp của giáo viên này
...
# 1. Trang danh sách quản lý giáo viên & phân công (Dành cho BGH)
...
# 2. Xử lý đổi Tổ trưởng / Cập nhật vai trò trực tiếp
...
def assign_role():
    ...
    if teacher:
        ...
        if new_role_code == "TT" and new_dept_id:
            ...
            # Cập nhật tổ và vai trò bình thường
...
# 1. Thêm Giáo viên mới (Xử lý gán subject_id)
@app.route("/admin/add-teacher", methods=["POST"])
def add_teacher():
    ...
    # Nếu chưa chọn môn học, tự động lấy ID của môn học đầu tiên trong CSDL để tránh lỗi NOT NULL
    if not subject_id:
    ...
    new_teacher = Teacher(
        ...
        subject_id=subject_id  # Gán subject_id
    )
    db.session.add(new_teacher)
    db.session.commit()

    # Tự động gán bộ tiêu chí cho giáo viên mới
...
def delete_teacher(teacher_id):
    ...
    if teacher:
        # Xóa toàn bộ dữ liệu tiêu chí & minh chứng liên quan
